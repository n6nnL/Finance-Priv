...
```
src/
  config.js          # env валидаци (config/loadEnv.js-ээр .env уншина)
  logger.js          # pino logger + notifyError() hook
  db.js              # node:sqlite: lastSeenUid, Message-ID, гүйлгээ (push tracking)
  imap-client.js     # IMAP IDLE, reconnect, token refresh, catch-up
  parsers/golomt.js  # Голомт parser (5 формат) → tx object
  categorize.js      # keyword ангилагч (config-ийн matchByKeywords)
  push.js            # API руу POST + retry
  index.js           # бүгдийг холбосон entry point
config/              # ★ дундын эх сурвалж (listener + api + discord импортолдог)
  categories.js      # ангилал + keyword + CATEGORY_META + matchByKeywords
  txfields.js        # detectIsPos, isoDate (parser + api дундын)
  loadEnv.js         # .env унших нэг shared parser
scripts/
  get-token.js          # Gmail OAuth refresh_token авах
  repush.js             # push_failed re-push
  migrate-categories.js # хуучин→канон ангилал миграц (dry-run default)
  recategorize.js       # дүрмээр дахин ангилах
  reparse.js            # дахин parse
test/
  golomt.test.js
  categorize.test.js
  shared.test.js        # дундын функц (txfields/categories) characterization
ecosystem.config.cjs    # pm2 (listener + api + discord)
```
