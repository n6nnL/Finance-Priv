...
// ---- Dashboard нэр/нууц үг нэвтрэлт ----
test('login: буруу нууц үг → 401, зөв → token, токеноор API ажиллана', async () => {
  const d = createDb(':memory:');
  const app = createApp({ db: d, ai: mockAi, apiKey: API_KEY, dashboardUser: 'admin', dashboardPassword: '2580', rateLimit: { windowSeconds: 60, max: 100000 } });
  const srv = await new Promise((r) => { const s = app.listen(0, () => r(s)); });
  const url = `http://127.0.0.1:${srv.address().port}`;
  const LH = { 'Content-Type': 'application/json' };

  const bad = await fetch(url + '/api/login', { method: 'POST', headers: LH, body: JSON.stringify({ username: 'admin', password: 'wrong' }) });
  assert.equal(bad.status, 401);

  const ok = await fetch(url + '/api/login', { method: 'POST', headers: LH, body: JSON.stringify({ username: 'admin', password: '2580' }) });
  assert.equal(ok.status, 200);
  const { token } = await ok.json();
  assert.ok(token && token.length >= 32);

  // dashboard token нь X-API-Key болж ажиллана
  assert.equal((await fetch(url + '/api/categories', { headers: { 'X-API-Key': token } })).status, 200);
  // бодит API key (listener) ХЭВЭЭР ажиллана
  assert.equal((await fetch(url + '/api/categories', { headers: { 'X-API-Key': API_KEY } })).status, 200);
  // токенгүй → 401
  assert.equal((await fetch(url + '/api/categories')).status, 401);
